#!/usr/bin/env python
"""
DistilBERT Enclave Performance Profiler.

This script measures DistilBERT layer execution in Enclave mode:
- All layers (Linear, LayerNorm, Softmax, GELU, MatMul, ReLU) run in Enclave

Usage:
    LD_PRELOAD=/usr/lib/x86_64-linux-gnu/libstdc++.so.6 python -m experiments.models.profile_distilbert_enclave

DistilBERT Key Features:
- 6 layers (50% fewer than BERT-base)
- No segment embeddings
- Pre-classifier with ReLU (instead of GELU)
...
Output: experiments/data/distilbert_{variant}_enclave_layers.csv
"""
...
class DistilBERTEnclaveProfiler:
    """Profiler for DistilBERT layers in Enclave mode."""
        ...
        if self.reuse_single_enclave:
            try:
                ...
                for block_idx in range(self.num_layers):
                    if verbose:
                        print(f"\n--- Encoder {block_idx} (Layer {block_idx+1}/6) ---")
    ...
    def _profile_relu_enclave(
        ...
        try:
            ...
            relu_layer = SecretReLULayer(
                sid, lname("relu"),
                ExecutionModeOptions.Enclave,
                manually_register_prev=True,
                manually_register_next=True
            )
            relu_layer.register_prev_layer(input_layer)
            ...
            output_layer.register_prev_layer(relu_layer)
            layers.append(output_layer)
    ...
    def print_summary(self):
        ...
        print(f"\n{'='*60}")
        print(f"DistilBERT-{self.model_variant.capitalize()} Enclave Profiling Summary")
        ...
        print_memory_summary(self.metrics, "DistilBERT Enclave Memory Analysis")
...
def main():
    parser = argparse.ArgumentParser(description='Profile DistilBERT layers in Enclave')
    ...
    profiler = DistilBERTEnclaveProfiler(
