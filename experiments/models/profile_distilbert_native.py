"""
DistilBERT Performance Profiler for Distributed Inference Modeling.
...
Output: experiments/data/distilbert_base_layers.csv

DistilBERT Architecture Summary (6 layers - 50% fewer than BERT):
- Embedding: Token + Position Embedding (NO segment embedding)
- Each Encoder Block: 
  - LayerNorm -> Attention (Q/K/V proj, MatMul, Softmax, Out proj)
  - Residual -> LayerNorm -> FFN (FC1, GELU, FC2) -> Residual
- Pre-classifier: Linear + ReLU
- Classifier: Linear

Reference: "DistilBERT, a distilled version of BERT" (Sanh et al., NeurIPS 2019)
"""
...
DISTILBERT_CONFIG = {
    'base': {
        'embed_dim': 768,
        'num_heads': 12,
        'num_layers': 6,  # Key difference: 6 layers (vs BERT's 12)
...
class DistilBERTPureProfiler:
    """
    Pure PyTorch profiler for DistilBERT models.
    ...
    def __init__(
        ...
        config = DISTILBERT_CONFIG.get(model_variant, DISTILBERT_CONFIG['base'])
        self.embed_dim = config['embed_dim']
        self.num_heads = config['num_heads']
        self.num_layers = config['num_layers']  # 6 layers!
    ...
    def _create_pytorch_layers(self):
        ...
        # Encoder layers (only 6 layers in DistilBERT!)
        ...
        # Pre-classifier (DistilBERT specific: Linear + ReLU)
        layers['pre_classifier'] = nn.Linear(self.embed_dim, self.embed_dim)
    ...
    def profile_cpu(self, verbose: bool = True) -> Dict[str, LayerMetrics]:
        ...
        if verbose:
            print(f"\n{'='*60}")
            print(f"Profiling DistilBERT-{self.model_variant.capitalize()} on CPU")
        ...
        with torch.no_grad():
            ...
            # ===== Pre-classifier and Classifier =====
            if verbose:
                print(f"\n--- Classification Head ---")
            
            # Pre-classifier (Linear + ReLU) - DistilBERT specific
            cls_token = torch.randn(self.batch_size, self.embed_dim)
            self._profile_layer(
                'pre_classifier', layers['pre_classifier'], cls_token,
                input_shape=[self.batch_size, self.embed_dim],
                output_shape=[self.batch_size, self.embed_dim],
                group='ClassHead', verbose=verbose
            )
            
            # ReLU (DistilBERT uses ReLU in pre-classifier)
            self._profile_relu(
                'pre_classifier_relu', cls_token,
                input_shape=[self.batch_size, self.embed_dim],
                output_shape=[self.batch_size, self.embed_dim],
                group='ClassHead', verbose=verbose
            )
    ...
    def _profile_relu(self, name: str, x, input_shape, output_shape, group: str, verbose: bool):
        """Profile ReLU activation (DistilBERT uses ReLU in pre-classifier)."""
        import torch
        import torch.nn.functional as F
    ...
    def save_results(self, output_dir: str = 'experiments/data'):
        ...
        csv_path = os.path.join(output_dir, f'distilbert_{variant}_layers.csv')
        json_path = os.path.join(output_dir, f'distilbert_{variant}_layers.json')
        ...
        with open(json_path, 'w') as f:
            json.dump({
                'model_config': {
                    ...
                    'note': 'DistilBERT has 6 layers (50% of BERT-base)',
                },
                'profiling_config': {
                    'num_iterations': self.num_iterations,
    ...
    def print_summary(self):
        ...
        print(f"\n{'='*60}")
        print(f"DistilBERT-{self.model_variant.capitalize()} Profiling Summary")
        print(f"  (6 layers - 50% fewer than BERT-base)")
        print(f"{'='*60}")
        ...
        print_memory_summary(self.metrics, "DistilBERT Memory Analysis")


def main():
    parser = argparse.ArgumentParser(description='Profile DistilBERT model layers on CPU')
    parser.add_argument('--model', type=str, default='base',
                       choices=['base'],
                       help='DistilBERT model variant')
    ...
    profiler = DistilBERTPureProfiler(
